...
const Projects = () => {
  ...
  return (
    <main ref={container} className="relative bg-white">
      ...
      <div className="h-screen flex items-center justify-center relative overflow-hidden">
        ...
        >
          ...
          <motion.div
            ...
            className="mt-16"
          >
            <ChevronRight className="h-8 w-8 text-blue-500 mx-auto rotate-90" />
          </motion.div>
        </motion.div>
      ...
      {/* Project Sections */}
      <div className="relative h-[400vh] bg-blue-50">
      ...
      {/* Footer CTA */}
      <div className="py-28 px-6 bg-gradient-to-b from-blue-50 to-white">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="px-10 py-4 rounded-xl cursor-pointer bg-gradient-to-r  from-blue-400 to-blue-600 text-white font-semibold text-lg"
          >
            Get in Touch
    ...
    >
      <div className={`relative rounded-3xl shadow-lg flex flex-col md:flex-row overflow-hidden w-[90%] max-w-6xl h-[70%] border-2 ${colors[color]}`}>
        ...
        <div className={`w-full md:w-1/2 p-10 flex flex-col justify-center space-y-6 bg-white z-10 ${position === "right" ? "order-2" : ""}`}>
          <div className="text-blue-500 font-mono text-sm mb-2">FEATURED PROJECT</div>
          <h2 className="text-4xl font-bold text-gray-800">{title}</h2>
          <p className="text-gray-600 text-lg leading-relaxed">{description}</p>
          
          <div className="mt-4">
            <div className="text-sm text-blue-500 mb-3">TECHNOLOGIES USED</div>
            <div className="flex flex-wrap gap-3">
              {techStack.map((tech, i) => (
                <span key={i} className="px-3 py-1.5 bg-blue-50 text-blue-600 rounded-full text-sm border border-blue-100">
                  {tech}
                </span>
              ))}
            </div>
          ...
          <motion.button
            whileHover={{ x: 5 }}
            className="flex items-center mt-8 text-blue-500 group"
          >
        ...
        <div className="w-full md:w-1/2 h-full relative">
          ...
          />
          <div className="absolute bottom-6 right-6 bg-white/80 rounded-full px-3 py-1 text-sm text-blue-600 backdrop-blur-sm border border-blue-100">
            Interactive Demo
          </div>
        </div>
      </div>
